...
- **목표**: 공식 CLI의 `validate`·`patch`·diff 스캔에 대응하는 전역 스킬 3종을 추가한다: `/security-validate-local`, `/security-patch-local`, `/security-diff-scan-local`.
...
**`/security-validate-local`** (Phase 1만 필요)
...
**`/security-patch-local`** (Phase 1만 필요)
...
**`/security-diff-scan-local`** (Phase 2 필요)
...
- `/security-patch-local`과 Phase 2 이력의 연동(occurrence 지정 수정): 워크벤치 remediation 프로토콜(action token, expected version) 재현이 필요해 별도 작업으로 분리.
...
- KTD1. **스킬 3개로 분리** — 단일 스킬에 서브커맨드를 두는 대신 Claude Code 스킬 디스커버리(description 트리거)에 맞게 목적별 스킬로 나눈다. 공용 스크립트는 `skills/security-scan-local/scripts/`를 재사용한다.
...
### U2. /security-patch-local

- **Goal**: 보안 이슈 수정 스킬을 2단 승인 구조로 완성한다.
- **Requirements**: R4, R5, R6, R7
- **Dependencies**: Phase 1 U2 (bootstrap), Phase 1 U3 (번역 규칙의 축소판 재사용)
- **Files**: `skills/security-patch-local/SKILL.md`
- **Approach**:

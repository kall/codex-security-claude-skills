...
**검증일**: 2026-07-30
**산출물**: `skills/security-scan-match-local/`(SKILL.md + scripts/validate_matches.py),
`skills/security-deep-scan-local/SKILL.md`, `docs/verification/phase4-deep-lite-pipeline.md`(U2 실측)
...
### U1. /security-scan-match-local — 매칭
...
### U3. /security-deep-scan-local — deep-lite
...
### U4. 종단 검증
- **install.sh 6개 스킬 전부 설치 확인**(security-scan-local / -validate-local / -patch-local /
...
|---|---|---|---|
| `scan`(전체) | `/security-scan-local` | 불필요 | ✓ (Phase 1, U6 종단 실증) |
...
| `validate` | `/security-validate-local` | 불필요 | ✓ (Phase 3) |
| `patch` | `/security-patch-local`(2단 승인) | 불필요 | ✓ (Phase 3) |
| diff/working-tree 스캔 | `/security-diff-scan-local` | 불필요 | ✓ (Phase 3) |
| `scans match` | `/security-scan-match-local`(격리 서브에이전트) | 불필요 | ✓ (Phase 4) |
| deep 스캔 | `/security-deep-scan-local`(deep-lite) | 불필요 | ✓ 축소판 (Phase 4) |
